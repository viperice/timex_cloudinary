...
/* Local includes */
var data = require('int_cloudinary_custom/cartridge/scripts/data/cloudinaryData');
var calls = require('int_cloudinary/cartridge/scripts/lib/calls');
 ...
 * @returns {object} 
 */
function getUrlForProductImage(templateProduct, image, size, useSrcset, customSrcset, siteName) {
	if (data.isCloudinaryEnabled()) {
		var folder = 'Catalogs/';
		var cat;
		var masterProduct;
		...
		var imageURL = image.URL.toString().split('?')[0];
		...
		var prodTileTransformationsFree = '';
		var productImageDropdown ='';
		
		if(session.custom.siteID != 'archive' && session.custom.siteID == 'timex' && siteName == null){
			prodTileTransformationsFree = JSON.parse(dw.system.Site.getCurrent().getCustomPreferenceValue('CloudinaryProductImageTransformationsFree')).transformations + '/';
			productImageDropdown = data.getProductImageDropdowns();
		}else if(session.custom.siteID == 'gcwatches' || session.custom.siteID == 'teslarus' || session.custom.siteID == 'guesswatches'){
			prodTileTransformationsFree = JSON.parse(dw.system.Site.getCurrent().getCustomPreferenceValue('CloudinaryProductImageTransformationsFree')).transformations + '/';
			productImageDropdown = data.getProductImageDropdowns();
		}else {
			prodTileTransformationsFree = transformations;
			productImageDropdown = data.getImageDropdowns();
		}
		...
		if (useSrcset) {
			...
			if (seoSuffix != '') {
				...
				clearSize = srcsetObject.clearSize;
			} else {
				imageURL = getCloudinaryURL() + '/image/upload/' + productImageDropdown + prodTileTransformationsFree + imageSize + folder + imageCatalog + imageId + imageType;
				var srcsetObject = getSrcsetForProductImage(getCloudinaryURL() + '/image/upload/' + productImageDropdown + prodTileTransformationsFree, '/' + folder + imageCatalog + imageId + imageType, customSrcset);
				srcset = srcsetObject.text;
				clearSrcset = srcsetObject.clearSrcset;
				clearSize = srcsetObject.clearSize;
			}
...
/**
 * Copy of getUrlForProductImage, but only for the PLP
 * Returns URL for Product image
 * @param {Object} templateProduct 
 * @param {Object} image 
 * @param {string} size 
 ...
 * @returns {object} 
 */
function getUrlForPLPImage(templateProduct, image, size, useSrcset, customSrcset) {
	if (data.isCloudinaryEnabled()) {
		var folder = 'Catalogs/';
		var cat;
		var masterProduct;
		...
		if (CloudinaryGlobalImageSettings) {
			if (transformations == '' && CloudinaryGlobalImageSettings.transformations) {
				if (size = 'tile')
					transformations = CloudinaryGlobalImageSettings.transformations + ',e_unsharp_mask:70' + '/';
				else
					transformations = CloudinaryGlobalImageSettings.transformations + '/';
			}
			if (seoSuffix == '' && CloudinaryGlobalImageSettings.seoSuffix) {
		...
		if(session.custom.siteID != 'archive' && session.custom.siteID == 'timex'){
			prodTileTransformationsFree = JSON.parse(dw.system.Site.getCurrent().getCustomPreferenceValue('CloudinaryProductImageTransformationsFree')).transformations + '/';
			productImageDropdown = data.getProductImageDropdowns();
		}else if(session.custom.siteID == 'gcwatches' || session.custom.siteID == 'teslarus' || session.custom.siteID == 'guesswatches'){
			prodTileTransformationsFree = JSON.parse(dw.system.Site.getCurrent().getCustomPreferenceValue('CloudinaryProductImageTransformationsFree')).transformations + '/';
		...
		if (useSrcset) {
			...
			if (seoSuffix != '') {
				...
				if (size = 'tile')
					imageURL = getCloudinaryURL() + '/image/upload/' + data.getPLPImageDropdowns() + transformations + imageSize + folder + imageCatalog + imageId + imageType;
				else
					imageURL = getCloudinaryURL() + '/image/upload/' + data.getImageDropdowns() + transformations + imageSize + folder + imageCatalog + imageId + imageType;
			}
			return {imageUrl: imageURL};
...
function getUrlForContentSlot(params) {
	...
	var libraryId = !params.libraryId ? '' : params.libraryId;
	var contentId = !params.contentId ? '' : params.contentId;
	var transformations = !params.transformations ? '' : params.transformations + '/';
	var seoSuffix = !params.seoSuffix ? '' : params.seoSuffix;
	var size = !params.size ? '' : params.size;
	var imageSize = getImageSize(size);
	var customSrcset = !params.srcset ? '' : params.srcset;
	...
	if (data.isCloudinaryEnabled()) {
		...
		if (customSrcset !== '') {
			...
			if (seoSuffix != '') {
				...
				var srcsetObject = getSrcsetForProductImage(getCloudinaryURL() + '/images/' + data.getImageDropdowns() + transformations, '/' + folder + libraryId + '/' + imageId + '/' + seoSuffix + imageType, customSrcset);
				srcset = srcsetObject.text;
				clearSrcset = srcsetObject.clearSrcset;
			} else {
				imageURL = getCloudinaryURL() + '/image/upload/' + data.getImageDropdowns() + transformations + imageSize + folder + libraryId + '/' + imageId;
				var srcsetObject = getSrcsetForProductImage(getCloudinaryURL() + '/image/upload/' + data.getImageDropdowns() + transformations, '/' + folder + libraryId + '/' + imageId, customSrcset);
				srcset = srcsetObject.text;
				clearSrcset = srcsetObject.clearSrcset;
			}
			var xyz = '123';
	...
	if (data.isCloudinaryEnabled()) {
		...
		if (SrcsetSizes) {
			...
			for (var i = 0; i < keys.length; i++) {
				...
				if (!!SrcsetSizes[keys[i]].style && SrcsetSizes[keys[i]].style !== '') {
					if (i > 0) {
						sizesText = sizesText + ", ";
					}
 ...
 * @param {string} size // image size for img src
 * @param {string} srcset // predefined object from srcset.js
 * @param {string} transformations
 * @param {string} seoSuffix
 * @returns {Object}
 */
function getImgUrlForContentSlot(image, libraryId, contentId, size, srcset, transformations, seoSuffix) {
	var imgURLString = image.URL.toString();
	var imgURL = imgURLString.split('default/dw')[1];
	var cutAt = imgURL.indexOf('/')+1;
	var imgId = imgURL.slice(cutAt);
	...
	var imageURL = getUrlForContentSlot(imageObj);
	return imageURL;
}
/**
 * Target only the mobile images
 */
function getMobileUrlForPLPImage(templateProduct, image, size, useSrcset, customSrcset) {
	if (data.isCloudinaryEnabled()) {
		var folder = 'Catalogs/';
		var cat;
		var masterProduct;
		...
		if(session.custom.siteID != 'archive' && session.custom.siteID == 'timex'){
			prodTileTransformationsFree = JSON.parse(dw.system.Site.getCurrent().getCustomPreferenceValue('CloudinaryProductImageTransformationsFree')).transformations + '/';
			productImageDropdown = data.getMobileProductImageDropdowns();
		} else {
			prodTileTransformationsFree = transformations;
			productImageDropdown = data.getImageDropdowns();
		}
...
module.exports.getUrlForPLPImage = getUrlForPLPImage;
module.exports.getMobileUrlForPLPImage = getMobileUrlForPLPImage;
...
module.exports.getImgUrlForContentSlot = getImgUrlForContentSlot;
